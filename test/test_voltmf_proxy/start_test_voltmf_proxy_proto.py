...
from proto_messages_proxy import getSampleProtoProxy as getSampleProto
...
def main():
    kafka_if = kafka_proto_interface.KafkaProtoInterface(v_omci=None)
...
def create_onu_rpc():
    ...
    kafka_if = kafka_proto_interface.KafkaProtoInterface(name='mock1', v_omci=mock_vomci)
    voltmf_thread = threading.Thread(name='kafka_test_voltmf', target=kafka_if.start)
    voltmf_thread.start()
    while kafka_if._producer is None:
        time.sleep(10)

    kafka_if._producer.set_topics(['PROXY_REQUEST'])
    kafka_if._consumer.set_topics(['vomci-proxy-response'])
    ...
    kafka_if._producer.set_topics(['vomci-proxy-request'])
